...
createApp({
  ...
  mounted() {
    ...
    tg?.expand();
    this.loadUserData(); // Load initial data
  },
  methods: {
    initTelegram() {
      ...
      this.user = {
        ...
        avatar:
          ...
          "https://i.ibb.co/wF1p77Bn/cbb39453-fd3c-40d6-a3a4-ddba7e7b4bae.png",
      };
    },
    selectMethod(method) {
    ...
    async payWithTON() {
      ...
      try {
        ...
        const transaction = {
          ...
          messages: [{ address: this.tonWallet, amount: amountInNanotons, payload }],
        };
        console.log("[TON Connect] Sending transaction:", transaction);
        const result = await tonConnectUI.sendTransaction(transaction);
        if (result) {
          const transactionId = result.boc || Date.now().toString();
          await this._makePostRequest(
            "processDonation",
            {
            ...
            (data) => {
              if (data.success) {
                ...
                this.loadUserData(); // Refresh backend data
              } else {
    ...
    async _makePostRequest(command, params, callback) {
      console.log("Sending request to:", this.apiUrls[command], "with:", params);
      try {
        const response = await fetch(this.apiUrls[command], {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (callback) {
          callback(data);
          console.log("Callback executed for", command);
        }
      } catch (error) {
    ...
    loadUserData() {
      console.log("Loading data from:", this.loadUrl);
      fetch(this.loadUrl)
